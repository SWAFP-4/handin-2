module group_project.Functions
...
// 3. Set fitness goals
let setFitnessGoal userId goalType targetValue currentValueOpt deadlineOpt =
    let now = DateTime.Now
    let currentValue = defaultArg currentValueOpt 0.0
    let status =        
        if currentValue >= targetValue then 
            GoalStatus.Completed        
        else
            match deadlineOpt with
            | Some deadline when now <= deadline -> Active
            | Some _ -> Failed
            | None -> Active
    {
        UserId = userId
        ...
        CurrentValue = defaultArg currentValueOpt 0.0
